...
public class LeftConeScoreCubeGrabCube extends SequentialCommandGroup {
    
    public LeftConeScoreCubeGrabCube(SwerveSys swerveSys, LiftSys liftSys, ClawSys clawSys, IntakeSys intakeSys) {
        super(
            ...
            new WaitCmd(0.5),
            new OpenCmd(clawSys),
            new FollowTrajectoryCmd("LeftScoreCube1ToGrabCube2", swerveSys).alongWith(
                new WaitUntilCmd(() -> swerveSys.getPose().getX() > 6.0)
                .andThen(new OutCmd(intakeSys).alongWith(new SetAbsoluteSpeedCmd(intakeSys)))
                .andThen(new WaitUntilCmd(() -> swerveSys.getPose().getX() < 6.75))
                .andThen(new InCmd(intakeSys).alongWith(new StopRollersCmd(intakeSys)))
